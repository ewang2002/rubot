 ...
 */
export const getTimeFromObj = (
    cDateTime: Date,
    c: IInternalCourseData,
    useStart: boolean
): Date => {
 ...
 * @returns {StringSelectMenuBuilder[]} The message select menu array, or `null` if something went wrong.
 */
export const getSelectMenusFromBuildings = (
    buildings: string[],
    uniqueId: string
    ...
    if (selectMenus.at(-1)!.options.length === 25) {
        ...
        selectMenus.at(-1)!.data.placeholder += " & End Process";
    }
...
export function getUsedClassrooms(cDateTime: Date, nextTime: number): ClassroomSection {
    ...
    const isFinalTime = allCourses.some(
        (x) =>
            x.day[0] === currDateStr &&
    ...
    for (const classroom of classrooms) {
        ...
        for (; i < sharedClasses.length; i++) {
            ...
            const identifier =
                `${classToCheck.subjCourseId}-${classToCheck.meetingType}` +
        ...
        for (; i < sharedClasses.length; i++) {
            ...
            if (
                getTimeFromObj(cDateTime, classToCheck, true).getTime() - cDateTime.getTime() <=
                nextTime
            ) {
